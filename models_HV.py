...
class mgn_GRUODECell(nn.Module):
    def __init__(self, input_size, n_dim, bias=True):
        ...
        self.W_r_c = Parameter(torch.eye(input_size).repeat_interleave(n_dim, dim=1))
        self.W_z_c = Parameter(torch.eye(input_size).repeat_interleave(n_dim, dim=1))
        self.W_h_c = Parameter(torch.eye(input_size).repeat_interleave(n_dim, dim=1))
        self.U_r_c = Parameter(torch.eye(input_size * n_dim))
        self.U_z_c = Parameter(torch.eye(input_size * n_dim))
        self.U_h_c = Parameter(torch.eye(input_size * n_dim))
    ...
    def forward(self, t, mgn_h):
        mgn_x = torch.zeros(mgn_h.shape[0], self.input_size).to(mgn_h.device)
        mgn_r = torch.sigmoid(torch.mm(mgn_x, self.W_r_c) + torch.mm(mgn_h, self.U_r_c) + self.b_r_c)
        mgn_z = torch.sigmoid(torch.mm(mgn_x, self.W_z_c) + torch.mm(mgn_h, self.U_z_c) + self.b_z_c)
        mgn_h_tilde = torch.tanh(torch.mm(mgn_x, self.W_h_c) + torch.mm(mgn_r * mgn_h, self.U_h_c) + self.b_h_c)
...
class mgn_mGRUODECell(nn.Module):
    def __init__(self, input_size, n_dim, bias=True):
        ...
        self.W_z_c = Parameter(torch.eye(input_size).repeat_interleave(n_dim, dim=1))
        self.W_h_c = Parameter(torch.eye(input_size).repeat_interleave(n_dim, dim=1))
        self.U_z_c = Parameter(torch.Tensor(input_size * n_dim, input_size * n_dim))
        self.U_h_c = Parameter(torch.Tensor(input_size * n_dim, input_size * n_dim))
    ...
    def forward(self, t, mgn_h):
        mgn_x = torch.zeros(mgn_h.shape[0], self.input_size).to(mgn_h.device)
        mgn_z = torch.sigmoid(torch.mm(mgn_x, self.W_z_c) + torch.mm(mgn_h, self.U_z_c) + self.b_z_c)
        mgn_h_tilde = torch.tanh(torch.mm(mgn_x, self.W_h_c) + torch.mm(mgn_z * mgn_h, self.U_h_c) + self.b_h_c)
        mgn_dh = (1 - mgn_z) * (mgn_h_tilde - mgn_h)
...
class GRUCell(nn.Module):
    def __init__(self, input_size, n_dim, bias=True):
        ...
        self.W_r_d = Parameter(torch.eye(input_size).repeat_interleave(n_dim, dim=1))
        self.W_z_d = Parameter(torch.eye(input_size).repeat_interleave(n_dim, dim=1))
        self.W_h_d = Parameter(torch.eye(input_size).repeat_interleave(n_dim, dim=1))
        self.U_r_d = Parameter(torch.eye(input_size * n_dim))
        self.U_z_d = Parameter(torch.eye(input_size * n_dim))
        self.U_h_d = Parameter(torch.eye(input_size * n_dim))
    ...
    def forward(self, X, h):
        r = torch.sigmoid(torch.mm(X, self.W_r_d) + torch.mm(h, self.U_r_d) + self.b_r_d)
        z = torch.sigmoid(torch.mm(X, self.W_z_d) + torch.mm(h, self.U_z_d) + self.b_z_d)
        h_tilde = torch.tanh(torch.mm(X, self.W_h_d) + torch.mm(r * h, self.U_h_d) + self.b_h_d)
        h = z * h + (1 - z) * h_tilde
        return h, h_tilde
...
class mGRUCell(nn.Module):
    def __init__(self, input_size, n_dim, bias=True):
        ...
        self.W_z_d = Parameter(torch.eye(input_size).repeat_interleave(n_dim, dim=1))
        self.W_h_d = Parameter(torch.eye(input_size).repeat_interleave(n_dim, dim=1))
        self.U_z_d = Parameter(torch.eye(input_size * n_dim, input_size * n_dim))
        self.U_h_d = Parameter(torch.eye(input_size * n_dim, input_size * n_dim))
    ...
    def forward(self, X, h):
        z = torch.sigmoid(torch.mm(X, self.W_z_d) + torch.mm(h, self.U_z_d) + self.b_z_d)
        h_tilde = torch.tanh(torch.mm(X, self.W_h_d) + torch.mm(z * h, self.U_h_d) + self.b_h_d)
        h = z * h + (1 - z) * h_tilde
        return h, h_tilde
