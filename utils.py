...
def preprocessing(data, norm=True):
    data.columns = ['idx', 'time', 'ts_p1', 'ts_v1', 'ts_p2', 'ts_v2', 'ts_p3', 'ts_v3', 'ts_p4', 'ts_v4',
                    'ts_p5', 'ts_v5', 'ts_p6', 'ts_v6', 'ts_p7', 'ts_v7',
...
class option_dataset(Dataset):
    def __init__(self, args, ts):
        ...
        if self.model_name == 'Neural-CDE':
            sample_list = []
            num_series = int(np.sum([c.startswith("ts") for c in self.ts.columns]) / args.sub_series)
            for i in np.unique(self.ts['idx']):
                b = self.ts[self.ts['idx'] == i]
                b = b.reset_index(drop=True)
                idx = b['idx'].values[0]
                time_diff = set(np.round(np.arange(0.1, 11.1, 0.1), 2)) - set(np.round(b['time'], 2))
                # time_diff = set(np.round(np.arange(0, 1, 0.01), 2)) - set(np.round(b['time'], 2))
                for t in time_diff:
                    b.loc[b.shape[0]] = list(chain(*[[idx], [t], [0] * 3 * num_series]))
                b.sort_values(['idx', 'time'], inplace=True)
                b = b.reset_index(drop=True)
                if b.shape[0] != 110:
                    b = b[~b['time'].duplicated()]
                    b = b[b['time'] != 0]
                sample_list.append(b)

            sample_list = np.stack(sample_list)
            X_CDE = torch.FloatTensor(sample_list[:, :, 1:2*num_series + 2])
            M_CDE = torch.FloatTensor(sample_list[:, :, 2*num_series + 2:])
            M_CDE = M_CDE.repeat_interleave(2, dim=2)
            # impute the nan when no observations (mask == 0)
            X_CDE[np.concatenate([np.zeros([X_CDE.shape[0], X_CDE.shape[1], 1]) == 1, M_CDE == 0], axis=2)] = float('nan')
            self.coeffs = torchcde.linear_interpolation_coeffs(X_CDE, rectilinear=0)
    ...
    def __getitem__(self, idx):
        ...
        if self.model_name == 'Neural-CDE':
            batch_coeffs = self.coeffs[idx, :, :]
            return {'ts': batch_ts, 'ts_coeffs': batch_coeffs}
        else:
            return {'ts': batch_ts}
...
def option_collate_fn(batch):
    ...
    try:
        '''
        Process only for training Neural-CDE 
        '''
        batch_coeffs = torch.stack([b['ts_coeffs'] for b in batch])
    except:
        batch_coeffs = None
